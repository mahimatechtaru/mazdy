...
use App\Http\Controllers\Admin\SosLogController;
...
use App\Http\Controllers\Admin\KnowledgeCenterController;
use App\Http\Controllers\Admin\FranchiseMessageController;
...
Route::prefix('admin')->name('admin.')->group(function () {
    ...
    // SOS  Logs
    Route::controller(SosLogController::class)->prefix('sos-log')->name('sos.log.')->group(function () {
        Route::get('index', 'index')->name('index');
        Route::get('pending', 'pending')->name('pending');
        Route::get('complete', 'complete')->name('complete');
        Route::get('canceled', 'canceled')->name('canceled');
    ...
    Route::controller(UserCareController::class)->prefix('users')->name('users.')->group(function () {
        ...
        Route::get('assigned_provider/{username}', 'assigned_provider')->name('assigned_provider');
        Route::post('assign_provider/{username}', 'assign_provider')->name('assign_provider');
    });
    ...
    // Vendor Care Section
    Route::controller(VendorCareController::class)->prefix('vendors')->name('vendors.')->group(function () {
        Route::get('index', 'index')->name('index');
        Route::get('active', 'active')->name('active');
        Route::get('banned', 'banned')->name('banned');
        Route::get('email/unverified', 'emailUnverified')->name('email.unverified');
        ...
        Route::get('kyc/unverified', 'KycUnverified')->name('kyc.unverified');
        Route::get('kyc/details/{username}', 'kycDetails')->name('kyc.details');
        Route::get('email-vendors', 'emailAllVendors')->name('email.vendors');
        Route::post('email-vendors/send', 'sendMailVendors')->name('email.vendors.send')->middleware("mail");
        Route::get('details/{username}', 'vendorDetails')->name('details');
        Route::post('details/update/{username}', 'userDetailsUpdate')->name('details.update');
        Route::get('login/logs/{username}', 'loginLogs')->name('login.logs');
        Route::get('mail/logs/{username}', 'mailLogs')->name('mail.logs');
        Route::post('send/mail/{username}', 'sendMail')->name('send.mail')->middleware("mail");
    ...
    Route::controller(FranchiseMessageController::class)->prefix('franchise/message')->name('franchise.messages.')->group(function () {
        Route::get('index', 'index')->name('index');
        Route::post('reply', 'reply')->name('reply');
        Route::get('export', 'export')->name('export');
        Route::delete('delete', 'delete')->name('delete');
    ...
    // franchise message Section
    Route::controller(KnowledgeCenterController::class)->prefix('knowledge_center/message')->name('knowledge_center.messages.')->group(function () {
        Route::get('index', 'index')->name('index');
    ...
    Route::controller(FaqController::class)->prefix('faq')->name('faq.')->group(function () {
        ...
        Route::put('update/{item}', 'itemUpdate')->name('update');
        Route::delete('delete/{item}', 'itemDestroy')->name('delete');
        Route::put('status-update/{item}', 'itemStatusUpdate')->name('status.update');
    });

    // ===============================================
    //               1. FAQ Category Routes
    // ===============================================

    Route::controller(FaqController::class)->prefix('categories')->name('category.')->group(function () {
        ...
        Route::delete('delete/{category}', 'categoryDestroy')->name('delete');

        // Route::put('status-update/{category}', 'categoryStatusUpdate')->name('status.update'); 
    });
});
