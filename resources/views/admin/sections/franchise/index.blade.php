...
@section('breadcrumb')
    @include('admin.components.breadcrumb', [
        ...
        'active' => __('franchise Messages'),
    ])
@endsection
@section('content')
    <div class="table-area">
        <div class="table-wrapper">
            <form method="POST" action="{{ setRoute('admin.Franchise.messages.delete.all') }}">
                @csrf
                <div class="table-header">
                    <h5 class="title">{{ __($page_title) }} <span
                            class="badge badge--success">{{ $franchisee_applications->total() }}</span> </h5>
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn--base bg--danger mark-delete-btn">{{ __('Mark Delete') }}</button>
                        <a href="{{ setRoute('admin.franchise.messages.export') }}"
                            class="btn--base bg--success">{{ __('Export (Excel)') }}</a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="custom-table">
                        ...
                        <tbody class="mark-all-parent">
                            @forelse ($franchisee_applications ?? [] as $key => $item)
                                <tr
                                    data-item="{{ json_encode($item->only(['id', 'message', 'full_name', 'email', 'mobile_number', 'operating_centre', 'city', 'consent', 'created_at'])) }}">
                                    <td>
                                    ...
                                    <td>
                                        {{-- @include('admin.components.link.custom', [
                                            ...
                                            'permission' => 'admin.franchise.messages.reply',
                                        ]) --}}
                                        @include('admin.components.link.custom', [
                                            'href' => '#details',
                                            'class' => 'btn btn--base details-button modal-btn',
                                            'icon' => 'las la-info-circle',
                                            'permission' => 'admin.franchise.messages.index',
                                        ])
                                        @include('admin.components.link.custom', [
                                            'href' => 'javascript:void(0)',
                                            'class' => 'btn btn--base bg--danger delete-button',
                                            'icon' => 'las la-trash',
                                            'permission' => 'admin.franchise.messages.delete',
                                        ])
                                    </td>
                                </tr>
                            @empty
        ...
        {{ get_paginate($franchisee_applications) }}
    </div>

    {{-- Send Mail Modal --}}
    {{-- @if (admin_permission_by_name('admin.franchise.messages.reply'))
        <div id="send-reply" class="mfp-hide large">
            <div class="modal-data">
                ...
                <div class="modal-form-data">
                    <form class="modal-form" action="{{ setRoute('admin.franchise.messages.reply') }}" method="POST">
                        @csrf
    ...
    @if (admin_permission_by_name('admin.franchise.messages.index'))
        <div id="details" class="mfp-hide large">
            <div class="modal-data">
                <div class="modal-header px-0">
                    <h5 class="modal-title">{{ __('Franchise Message') }}</h5>
                </div>
...
@push('script')
    <script>
        ...
        $(".details-button").click(function() {
            ...
            <div class = "user-info">
                <ul class = "border-bottom pb-3">
                    <li>
                        <strong>Name: </strong> <span>${message.full_name}</span>
                    </li>
                     <li>
                        <strong>Mobile: </strong> <span>${message.mobile_number}</span>
                    </li>
                    <li>
                        <strong>Email: </strong> <span>${message.email}</span>
                    </li>
                     <li>
                        <strong>Business Type: </strong> <span>${message.business_type}</span>
                    </li>
                     <li>
                        <strong>Operating Centre: </strong> <span>${message.operating_centre}</span>
                    </li>
                     <li>
                        <strong>City: </strong> <span>${message.city}</span>
                    </li>
                     <li>
                        <strong>Consent: </strong> <span>${message.consent}</span>
                    </li>   
                    <li>
        ...
        $(".delete-button").click(function() {
            ...
            var actionRoute = "{{ setRoute('admin.franchise.messages.delete') }}";
